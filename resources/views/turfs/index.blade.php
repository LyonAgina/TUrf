    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-md shadow-sm p-2 mb-4">
            ...
            <style>
                ...
                @keyframes sparkle { 0% { opacity: 0.7; transform: scale(1) translateY(0); } 50% { opacity: 1; transform: scale(1.3) translateY(-8px); } 100% { opacity: 0.7; transform: scale(1) translateY(0); } }
            </style>
            ...
            <form method="GET" action="{{ route('turfs') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search turfs..."
                ...
                <select name="sport" class="px-2 py-1.5 text-sm bg-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 transition">
                ...
                <select name="location" class="px-2 py-1.5 text-sm bg-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 transition">
                    <option value="">All Regions</option>
                    @foreach(\App\Models\Location::select('city')->distinct()->orderBy('city')->get() as $loc)
                        <option value="{{ $loc->city }}" {{ request('location') == $loc->city ? 'selected' : '' }}>{{ $loc->city }}</option>
                    @endforeach
                </select>

                <select name="price" class="px-2 py-1.5 text-sm bg-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 transition">
                ...
                <div class="flex gap-2 mt-2 lg:mt-0">
                    ...
                    <a href="{{ route('turfs') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-semibold hover:bg-gray-300 transition">Clear</a>
                </div>
            </form>
            ...
            @forelse($turfs as $turf)
                <div class="group relative bg-white rounded-2xl shadow-md hover:shadow-2xl transform hover:scale-[1.02] transition-all duration-300 overflow-hidden">
                    <div class="relative h-64 overflow-hidden bg-gray-200">
                        {{-- FIX: Use Picsum Photos for local development visibility --}}
    ...
    loading="lazy">
                    ...
                    <div class="p-6 bg-gradient-to-b from-transparent via-green-50 to-white group-hover:via-green-100 transition-colors duration-500">
                        ...
                        <div class="flex items-center gap-2 text-gray-600 mb-4">
                            ...
                            @if($turf->location)
                                <span>{{ $turf->location->city }}{{ $turf->location->neighborhood ? ', ' . $turf->location->neighborhood : '' }}</span>
                            @else
                        ...
                        <div class="flex flex-wrap gap-2 mb-5">
                            @if($turf->sport)
                                <span class="px-3 py-1 {{ $turf->sport === 'volleyball' ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800' }} rounded-full text-xs font-medium">
                                    {{ ucfirst($turf->sport) }}
                                </span>
                            @else
                                <span class="px-3 py-1 bg-gray-100 text-gray-500 rounded-full text-xs font-medium">Sport not set</span>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-green-200">
                            ...
                            <div class="flex gap-2">
                                @auth
                                    <a href="/bookings/create?turfID={{ $turf->turfID }}"
                                ...
                                @else
                                    <a href="{{ route('login') }}"
                                       class="bg-blue-600 hover:bg-blue-700 hover:scale-105 text-white font-bold py-3 px-6 rounded-xl transition-transform duration-300 text-sm whitespace-nowrap">
                                        Login to Book
                                    </a>
                                @endauth

                                @can('delete', $turf)
                                    <form method="POST" action="{{ route('turfs.destroy', $turf->turfID) }}" class="inline-block turf-delete-form">
                                        @csrf @method('DELETE')
                                        <button type="button" class="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-xl transition turf-delete-btn">
                                            Delete
                                        </button>
                                        <div class="hidden turf-delete-confirm bg-white border border-red-400 text-red-700 px-3 py-2 rounded-xl shadow-lg absolute z-10 mt-2">
                                            Confirm delete?
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.turf-delete-btn').forEach(btn => {
            btn.addEventListener('click', () => btn.closest('.turf-delete-form').querySelector('.turf-delete-confirm').classList.remove('hidden'));
        });
        document.querySelectorAll('.turf-cancel-btn').forEach(btn => {
            btn.addEventListener('click', () => btn.closest('.turf-delete-form').querySelector('.turf-delete-confirm').classList.add('hidden'));
        });
    });
</script>
@endsection